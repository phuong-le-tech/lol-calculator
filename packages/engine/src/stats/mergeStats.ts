...
/**
 * Merge champion base stats (at level) with aggregated item stats and rune stats.
...
export function mergeStats(
  ...
  const runes = runeStats || {};

  const bonusHp = (itemStats.hp || 0) + (runes.hp || 0);
  const bonusAd = (itemStats.ad || 0) + (runes.ad || 0);

  // Move speed: flat additive first, then percentage multiplier, then soft cap
  const flatMoveSpeed = champ.moveSpeed + (itemStats.moveSpeed || 0) + (runes.moveSpeed || 0);
  ...
  return {
    ...
    baseAd: champ.baseAd,
    bonusAd,
    ap: (itemStats.ap || 0) + (runes.ap || 0),
    armor: champ.armor + (itemStats.armor || 0) + (runes.armor || 0),
    mr: champ.mr + (itemStats.mr || 0) + (runes.mr || 0),
    attackSpeed: calcAttackSpeed(
      baseStats.attackSpeed,
      baseStats.attackSpeedRatio,
      baseStats.attackSpeedGrowth,
      level,
      (itemStats.attackSpeed || 0) + (runes.attackSpeed || 0)
    ),
    ...
    abilityHaste: (itemStats.abilityHaste || 0) + (runes.abilityHaste || 0),
